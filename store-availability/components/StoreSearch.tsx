  ...
  return (
    <div className="w-72 ">
      <Listbox value={store} onChange={setStore}>
        <Listbox.Button className="inline-flex justify-between w-72 rounded-md border bg-white py-3 px-4">
          <p className="mr-14 my-auto text-sm text-gray-400">{store} </p>
          <ChevronDownIcon className="-mr-1 ml-2 h-5 w-5 my-auto text-gray-400" />
        </Listbox.Button>

        <Listbox.Options className="w-48 bg-darkgray text-white py-2 absolute rounded-lg opacity-80">
          {stores.map((each) => (
            <Listbox.Option
              key={each}
              value={each}
